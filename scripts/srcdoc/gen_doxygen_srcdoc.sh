...
function usage
{
	printf "\n\nThis script generates Doxygen sourcecode documentation for the Fldigi suite programs."
...
# Extract ALL the patches from the branch, and save to a subdirectory
function do_patches
{
	cd HTML
		mkdir __git; cd __git
			git format-patch --summary --root # Create patches for ALL commits
			git log --stat --root > gitlog.txt # Dump the history of ALL commits
		cd ..
	cd ..
}
...
if [ ! -e ./doxyfile.txt ]; then
	printf "\n\nERROR: Doxygen configuration file: \"doxyfile.txt\" not found."
	printf "\n\n === ABORTING === \n\n"
	exit 1
fi
...
doxygen doxyfile.txt 	# Run Doxygen (also calls dot & mscgen)
