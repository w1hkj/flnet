...
size_t Socket::sendTo(const void* buf, size_t len)
{
...
#ifndef NDEBUG
	{
		...
		printf("HAP:%lX:%d count=%d buf=%s\n", host_addr, host_port, len, buf);
	}
#endif
		...
		if (r > 0) {
			...
			if (r == 0) {
				...
				if (errno != EAGAIN && errno != 0) {
					printf("errno = %d (%s) r %d buff %s\n", errno, strerror(errno), r, sp);
				}
				r = 0;
			}
		}
	...
	if(r > 0) { // To prevent loop back and except only from address x
		...
		if((srvr_port == host_port) && (srvr_to_addr == host_addr)) {
			if((srvr_addr == host_addr) && (local_port == host_port)) {
				printf("Loopback Warning: %X:%u\n", (unsigned int)host_addr, host_port);
				memset(buf, 0, len);
				return 0;
			}
		}
