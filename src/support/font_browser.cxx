	 ...
	 : Fl_Window(x, y, w, h, lbl)
{
	...
	btn_fixed = new Fl_Check_Button(345, 15, 18, 18, "Fixed");
	btn_fixed->callback(fb_callback, this);
	btn_fixed->value(0);

	btn_OK = new Fl_Return_Button(345, 40, 80, 25, "&OK");
	btn_OK->shortcut(0x8006f);
	btn_OK->callback(fb_callback, this);

	btn_Cancel = new Fl_Button(345, 70, 80, 25, "Cancel");
	btn_Cancel->labelsize(12);
	btn_Cancel->callback(fb_callback, this);

	btn_Color = new Fl_Button(345, 100, 80, 25, "Color");
...
// Initializations
	...
	if (instance == 0) {
		...
		if (numfonts > 256) numfonts = 256;  // fltk will segfault trying to draw a label font > 255
...
{
	char sys_fonts[100];
	std::cout << "Font #, Type, Name" << std::endl;
...
void find_fonts()
{
	if (pthread_create(&find_font_thread, NULL, find_fixed_fonts, NULL) < 0) {
		std::cout << "pthread_create find_fixed_fonts failed" << std::endl;
	}
	return;
}
