...
/** \mainpage Flnet Sourcecode Interactive Doxygen Documentation : 000GIT_REVISION000
 <div align="center"><img src="fldigi-psk.png" ></div>

 \section intro Introduction

  Welcome to the Flnet Interactive Doxygen Sourcecode Documentation!
  Here you'll find information useful for developing and debugging Flnet. 
  
  Flnet provides the Amateur Radio Net Control Station operator with a real time tool to assist him or her in managing the net activities.<BR>
  A single screen with multiple windows is used to allow rapid entry, search, pick and display of all stations calling in to the net.<BR>


  <BR>
  \section download To Download Flnet :
  <UL>
  <LI> To pull latest source with developer access: <B> git clone ssh://<YOUR-LOGIN>\@git.code.sf.net/p/fldigi/flnet fldigi-flnet</B></LI>
  <LI> To pull latest source with developer access behind proxy/firewall: <B> git clone https://<YOUR-LOGIN>\@git.code.sf.net/p/fldigi/flnet fldigi-flnet</B></LI>
  <LI> To pull latest source with no account, read-only: <B> git clone git://git.code.sf.net/p/fldigi/flnet fldigi-flnet</B></LI>
  </UL>
  ...
  <OL>
  <LI>Download the Flnet sourcecode as described above</LI>
